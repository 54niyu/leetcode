...
public:
	//non-recursive
	int countNodeNonRecursive(TreeNode* root){
		if (!root) return 0;
		TreeNode *ref = root;
		int left, right;
		int count = 0;
		while (ref){
			left = getLeftHight(ref->left);
			right = getLeftHight(ref->right);
	...
	int countNodes(TreeNode* root) {
		...
		int right = getLeftHight(root->right);
...
//	start = clock();
//	cout << s.countNodeNonRecursive(tree);
//	finish = clock();
//	totaltime = (double)(finish - start) / CLOCKS_PER_SEC;
//	cout << "\n此程序的运行时间为" << totaltime << "秒！" << endl;;
//	return 0;
